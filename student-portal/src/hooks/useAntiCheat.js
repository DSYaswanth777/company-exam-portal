...
export const useAntiCheat = (onDisqualified, isExamActive) => {
  ...
  const totalViolations = Object.values(localViolations).reduce(
    (sum, count) => sum + count,
    0,
  );

  const disqualifyStudent = useCallback(
    async (violationType, reason) => {
      ...
      try {
        ...
        const response = await studentService.disqualify(violationType, reason);
    ...
    async (violationType) => {
      ...
      setLocalViolations((prev) => {
        ...
        if (threshold !== null && newCount >= threshold) {
          console.log(
            `🚨 Threshold exceeded for ${violationType}: ${newCount} >= ${threshold}`,
          );
      ...
      try {
        ...
        const response = await studentService.recordViolation(violationType);
