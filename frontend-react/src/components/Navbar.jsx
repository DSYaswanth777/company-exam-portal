...
export default function Navbar() {
  ...
  const renderItem = (item, idx, isMobile = false) => {
    ...
    const extra =
      "text-sm text-slate-600 dark:hover:text-black font-bold px-3 py-2 rounded-md transition-colors";
        ...
        <div className="flex h-20 items-center justify-between">
          <div className="flex items-center gap-3">
            <NavLink
              to="/"
              onClick={closeMobile}
              className="flex items-center gap-2 text-3xl font-semibold bg-linear-to-r from-blue-600 via-purple-600 to-pink-600 bg-clip-text text-transparent"
          ...
          <nav className="hidden md:flex items-center gap-6">
            ...
            {!user && (
              <>
                <NavLink
                  to="/admin/login"
                  className="text-sm text-slate-600 dark:hover:text-black font-bold px-3 py-2 rounded-md transition-colors"
                >
                  Admin Login
                </NavLink>
                <NavLink
            ...
            {!user && (
              <>
                <NavLink
                  to="/admin/login"
                  onClick={closeMobile}
                  className="block px-3 py-2 text-base font-medium text-slate-700 hover:text-black"
                >
                  Admin Login
                </NavLink>
                <NavLink
            ...
            {user && (
              <>
                <NavLink
                  to={user.userType === "admin" ? "/admin" : "/company"}
                  onClick={closeMobile}
                  className="block px-3 py-2 text-base font-semibold text-slate-600 hover:text-black"
                >
                  Dashboard
                </NavLink>
                <button
